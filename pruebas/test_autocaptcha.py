# -*- coding: utf-8 -*-
"""AutoCaptcha: que la aplicacion este bien cableada y clique lo que debe.

Se comprueba el cableado, que no arrastra nada de macros ni de teclado, y que
detecta y clica sobre una reconstruccion de la captura real del cofre.
"""
import os
import sys
import threading
...
def p_no_pisa_a_golem():
    ...
    check("los archivos llevan su nombre",
          "captcha" in os.path.basename(C.CONFIG_PATH)
          and "captcha" in os.path.basename(C.LOG_PATH),
          f"{os.path.basename(C.CONFIG_PATH)}, {os.path.basename(C.LOG_PATH)}")
...
def p_solo_lo_del_captcha():
    ...
    try:
        ...
        check("y son las esperadas",
              sorted(k.name for k in C.HOTKEYS) == ["f12", "f2", "f6", "f8"],
              str(sorted(k.name for k in C.HOTKEYS)))
    finally:
        cerrar(app, root)
